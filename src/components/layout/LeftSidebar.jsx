...
export function LeftSidebar({
  ...
  useEffect(() => {
    ...
    listReports()
      .then((response) => {
        if (active) setReportCount(response.meta?.total ?? 0);
      })
      .catch(() => {
  ...
  return (
    <>
      ...
      {desktopVisible ? (
        ...
        >
          <div
            className={`mb-4 flex h-12 items-center border-b border-line ${expanded ? "justify-end" : "justify-center px-2"}`}
          >
            <button
              type="button"
              onClick={onToggle}
              className="inline-grid h-10 w-10 shrink-0 cursor-pointer place-items-center rounded-lg text-black transition-colors hover:bg-brand-100"
              aria-label={tr(
                expanded
          ...
          >
            {sections.map((section, index) => (
              <section
                key={section.label}
                className={`grid gap-1 ${index ? "border-t border-line pt-5" : ""}`}
              ...
              >
                ...
                {section.links.map(({ label, icon, to }) => (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `community-nav-label flex min-h-12 items-center rounded-lg transition-colors ${expanded ? "gap-3 px-3" : "justify-center px-2"} ${isActive ? "bg-brand-100 text-brand-800" : "text-black hover:bg-[#f2f5f8] hover:text-brand-800"}`
                    }
                  >
                    <Icon name={icon} size={19} className="shrink-0" />
                    <span
                      className={`min-w-0 overflow-hidden whitespace-nowrap transition-[max-width,opacity] duration-200 ease-in-out ${expanded ? "max-w-32 flex-1 opacity-100" : "max-w-0 opacity-0"}`}
                    >
                      {tr(label)}
                    </span>
                    {to === "/history" && unseenScanCount > 0 ? (
                      <span className="grid h-5 min-w-5 place-items-center rounded-full bg-risk-high px-1 text-[10px] font-bold text-white">
                    ...
                    to === "/reports/history" &&
                    reportCount !== null ? (
                      <span className="community-meta">{reportCount}</span>
                    ) : null}
                  </NavLink>
                ))}
              </section>
