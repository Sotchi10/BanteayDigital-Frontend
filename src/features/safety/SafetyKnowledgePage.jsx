...
export function SafetyKnowledgePage() {
  ...
  useEffect(() => {
    let active = true;
    listSafetyKnowledge()
      .then((response) => { if (active) setKnowledge(response.knowledge || []); })
      .catch((requestError) => { if (active) setError({ message: requestError.response?.data?.message || "We could not load Community Safety guidance.", status: requestError.response?.status }); })
  ...
  return <main className="min-w-0 px-5 py-5 sm:px-8" id="main-content"><div className="mb-6"><p className="m-0 text-xs font-bold uppercase tracking-[0.1em] text-brand-700">Stay informed</p><h1 className="mb-1 mt-1 text-2xl font-bold text-brand-900">Community Safety</h1><p className="m-0 max-w-2xl text-sm text-muted">Practical guidance to help you recognise common online scams and protect your information.</p></div>
    ...
    {!loading && error ? <ErrorState title="Could not load guidance" message={error.message} status={error.status} onRetry={() => window.location.reload()} /> : null}
    {!loading && !error && knowledge.length === 0 ? <EmptyState icon="book" title="No safety guidance published yet" message="Please check back soon." /> : null}
