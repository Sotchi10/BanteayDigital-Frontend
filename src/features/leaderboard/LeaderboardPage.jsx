  ...
  useEffect(() => {
    ...
    listCommunityPosts({ limit: 100 })
      ...
      .catch((requestError) => {
        if (active)
          setError(
            apiErrorMessage(requestError, "Could not load community activity."),
          );
      })
      .finally(() => {
    ...
    () =>
      Object.values(
        posts.reduce((result, post) => {
          ...
          const entry = result[author.id] || { ...author, count: 0 };
          entry.count += 1;
          result[author.id] = entry;
          return result;
        }, {}),
          ...
          <Card className="mt-3 overflow-hidden">
            <div className="border-b border-line bg-[#f8fbff] px-5 py-4">
              ...
              <p className="m-0 mt-1 text-sm text-muted">
                {tr(
                  "Ranked by published safety alerts currently available from the community API.",
                )}
              </p>
            </div>
            {contributors.length ? (
              <ol className="m-0 list-none divide-y divide-line p-0">
                {contributors.map((person, index) => (
                  ...
                  >
                    <span
                      className={`grid h-9 w-9 shrink-0 place-items-center rounded-full text-sm font-bold ${index === 0 ? "bg-[#fff3cf] text-[#966800]" : "bg-[#f2f5f8] text-muted"}`}
                    >
                      {index + 1}
                    </span>
                    <Avatar
                      name={person.name || "Community member"}
                      tone="indigo"
                    />
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <strong className="block truncate text-sm">
                          {person.name || tr("Community member")}
                        </strong>
                        {index === 0 ? (
                          <span
                            className="sm:hidden text-[#966800] shrink-0"
                            title={tr("Most alerts")}
                          >
              ...
              <div className="p-8 text-center text-sm text-muted">
                {tr("No published alert contributors yet.")}
              </div>
            )}
          </Card>
        </>
